...
class Wordlift_Jsonld_Endpoint_Test extends Wordlift_Unit_Test_Case {
	 ...
	 */
	private static function provideEncodedMetasCases() {
	...
	public static function provideEncodedMetas() {
		// permutate a test case for each of provided query strings
		foreach ( self::provideEncodedMetasCases() as $testTitle => $args ) {
			list($persisted_value, $search_strings, $shouldMatch) =
			...
			if ( isset( $search_strings ) && is_array( $search_strings ) ) {
				...
				foreach ( $search_strings as $search_string ) {
					++$cnt;
					$tmpKey = sprintf( '%s - case #%d', $testTitle, $cnt );
					yield $tmpKey => array(
						$persisted_value,
						$search_string,
						null !== $shouldMatch ? $shouldMatch : true,
					);
				}
			}
		}
	...
	/** @dataProvider provideEncodedMetas */
	public function testShouldHandleEncodedMeta( string $stored_url, string $search_string, bool $shouldMatch ) {
		$this->loadFixture(
			$stored_url,
		...
		$request = new WP_REST_Request(
			'GET',
			'/' . WL_REST_ROUTE_DEFAULT_NAMESPACE . '/jsonld/meta/' . urlencode( $metaKey )
		);
	 ...
	 * @return void
	 */
	private function loadFixture( $url, $metaKey ) {
		...
		add_post_meta( $postId, $metaKey, $url );
	}
}
